...
assign_part
    ...
    ;
// NOTE: the old `listAssign`/`blockAssign` alternatives here were removed.
// They started with EQ (`EQ LBRACK ...` / `EQ LBRACE ...`), which directly
// conflicted with `test` already consuming a plain `x = value` assignment
// internally via the `comparison` rule's EQ operator. That ambiguity meant
// list/dict literal assignments (e.g. products = [{"name": "Phone"}, ...])
// were being mis-parsed through this legacy, single-item-only path instead
// of through the real listAtom/dictAtom atoms, silently corrupting the AST.
// Real list and dict literals are now parsed as plain atoms (see `atom`
// below: #listAtom, #dictAtom), reached the normal way through
...
atom
    ...
    | LBRACE (dictItem (COMMA dictItem)* COMMA?)? RBRACE  #dictAtom
    | name                             #nameAtom
    | PRINT                            #printAtom
