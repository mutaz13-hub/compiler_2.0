...
// NOTE: `opened` already existed as a bracket-depth counter (incremented by
// LPAREN/LBRACE, decremented by RPAREN/RBRACE) but two things were missing,
// which meant multi-line list/dict literals like:
//   products = [
//       {"name": "Phone", "price": 300},
//       {"name": "Laptop", "price": 800}
//   ]
// could never parse: (1) LBRACK/RBRACK ('[' / ']') never touched the
// counter, and (2) NEWLINE never actually checked `opened` before firing -
// it always emitted a real NEWLINE token, even in the middle of an open
// bracket, which is exactly what real Python suppresses.
NEWLINE : ('\r'? '\n') SPACES? { if (opened > 0) skip(); } ;
WS : [ \t]+ -> channel(HIDDEN) ;
SPACES: [ \t]+ ;
LINE_JOIN : '\\' [ \t]* ('\r'? '\n') -> skip ;
